...
export async function getBusSchedule(city: string, bus:string) {
    ...
    const res = await fetch(`https://tdx.transportdata.tw/api/basic/v2/Bus/Schedule/City/${city}/${bus}?$select=Direction,RouteName,Frequencys,TimeTables&$filter=RouteName/Zh_tw eq '${bus}'&$format=JSON`, {
        headers: {
